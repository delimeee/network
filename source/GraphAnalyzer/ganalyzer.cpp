...
double GraphAnalyzer::lvalue_constraint(Graph& g,
...
double GraphAnalyzer::rvalue_constraint_station(
    Graph& g, std::unordered_set<size_t>& sub_graph) {
    double production{0};
...
double GraphAnalyzer::rvalue_constraint_line(
    Graph& g, std::unordered_set<size_t>& sub_graph) {
    double production{0};
    double consumption{0};
    for (auto& v : sub_graph) {
...
std::optional<std::pair<std::unordered_set<size_t>, double>>
GraphAnalyzer::heuristic(Graph& g, std::unordered_set<size_t> checked) {
    ...
    if (std::abs(lval - std::max(rval, rval_second)) >= ERROR &&
        lval < std::max(rval, rval_second)) {
        return std::make_optional(
            std::pair{checked, std::max(rval, rval_second)});
    }
    return std::nullopt;
}

double GraphAnalyzer::calc_connectivity(Graph& g,
...
GraphAnalyzer::greedy_best_first_search(Graph& g, size_t start) {
